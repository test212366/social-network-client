	...
	return (
		<><aside className={isSide ? `${c.sidebar} ${c.sidebar__open}` : c.sidebar}>
			...
			<NavLink to={`/profile:${id}`} className={c.td} activeClassName='none' onClick={() => handlerCloseSidebar()}>
				<div className={c.sec}>
					{user ?
						<img src={user.user ? `${user.user.userPhoto != 'false' ? `${user.user.userPhoto}` : "https://okeygeek.ru/wp-content/uploads/2020/03/no_avatar.png"}` : `${user.userPhoto != 'false' ? `${user.userPhoto}` : "https://okeygeek.ru/wp-content/uploads/2020/03/no_avatar.png"}`} alt="user" className={c.sidebar__user} />
						: ""}
