...
const Post = ({ setIsReposts, isReposts, photoPost, user, description, temp, name, data, likes, setCommentsItem, commentsItem, commentsItems, src, reposts, id = false, setIsComments, isComments, repostMess }) => {
	...
	const handlerLike = async () => {
		if (likedPost) {
			...
			console.log(currentUserLike.user.userName)

			const responce = await fetch('http://localhost:5000/api/user/handlerDelLike', {
				headers: {
					'Content-Type': 'application/json'
				},
				method: 'POST',
			...
			const responce = await fetch('http://localhost:5000/api/user/handler', {
				headers: {
					'Content-Type': 'application/json'
				},
				method: 'POST',
	...
	console.log(src)
	console.log(`${src != 'false' ? `${src}` : "https://okeygeek.ru/wp-content/uploads/2020/03/no_avatar.png"}`)
			...
			<div className={c.post__header}>
				<div className="post__logo">
					<img
						...
						height='43px'
						src={`${src != 'false' ? `${src}` : "https://okeygeek.ru/wp-content/uploads/2020/03/no_avatar.png"}`} alt="postIMG" />
				</div>
