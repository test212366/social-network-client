...
const Profile = ({ id, handlerMessage }) => {
	...
	useEffect(() => {
		const api = async () => {
			...
			if (myUser) {
				const api = async () => {
					const responce = await fetch('http://localhost:5000/api/user/giveAllPosts', {
						headers: {
							'Content-Type': 'application/json'
						},
						method: 'POST',
			...
			const resp = await fetch('http://localhost:5000/api/user/updateFriends', {
				headers: {
					'Content-Type': 'application/json'
				},
				method: 'POST',
	...
	const handlerAddFriend = async () => {
		const temp = jwtDecode(localStorage.getItem('user'))
		const responce = await fetch('http://localhost:5000/api/user/addFriend', {
			headers: {
				'Content-Type': 'application/json'
			},
			method: 'POST',
		...
		console.log(123321)
		if (data.message) {
			const responceI = await fetch('http://localhost:5000/api/user/getUser', {
				headers: {
					'Content-Type': 'application/json'
				},
				method: 'POST',
	...
	const createPostItem = async () => {
		...
		const temp = jwtDecode(localStorage.getItem('user'))
		const responce = await fetch('http://localhost:5000/api/user/createNewPost', {
			headers: {
				'Content-Type': 'application/json'
			},
			method: 'POST',
	...
	const handlerDeleteFriend = async () => {
		const responce = await fetch('http://localhost:5000/api/user/deleteUser', {
			headers: {
				'Content-Type': 'application/json'
			},
			method: 'POST',
	...
	const handlerSet = async () => {
		const responce = await fetch('http://localhost:5000/api/user/setStatus', {
			headers: {
				'Content-Type': 'application/json'
			},
			method: 'POST',
	...
	const fileUp = async e => {
		if (myCurrentUser.user.userPhoto != 'false') {
			const responce = await fetch('http://localhost:5000/api/user/deleteAvatar', {
				headers: {
					'Content-Type': 'application/json'
				},
				method: 'POST',
		...
		const responce = await fetch('http://localhost:5000/api/user/uploadAvatar', {
			method: 'POST',
			body: formData
		})
		const data = await responce.json()
	...
	const handlerUpload = async e => {
		const formData = new FormData()
		formData.append('file', e.target.files[0])
		const responce = await fetch('http://localhost:5000/api/user/uploadPhoto', {
			method: 'POST',
			body: formData
		})
		const data = await responce.json()
	...
	const handlerNewPhotoProfile = async e => {
		...
		formData.append('file', e.target.files[0])
		formData.append('id', `${myCurrentUser.user._id}`)
		const responce = await fetch('http://localhost:5000/api/user/uploadPhotoUs', {
			method: 'POST',
			body: formData
		})
		const data = await responce.json()
								...
								<div className={c.handler}>
									<div className={c.user__item}>
										<div className={c.user__photo} onClick={() => uploadPhoto()}>
											<img src={`${myUser.userPhoto != 'false' ? `${myUser.userPhoto}` : "https://okeygeek.ru/wp-content/uploads/2020/03/no_avatar.png"}`}
												width='70px'
												height='70px'
								...
								<div className={c.user__photo_item}>
									...
									<div className={c.photos}>
										{myUser ? myUser.photos.length === 1 ? <img
											...
											src={myUser.photos[0].src} alt="usersPhoto" /> : '' : ''}
										...
										{myUser ? myUser.photos.length === 2 ? <div>
											...
											<img
												...
												height='120px'
												className={c.user__photos_item}
												src={myUser.photos[1].src} alt="usersPhoto" />
										</div> : '' : ''}
										{myUser ? myUser.photos.length >= 3 ? <div>
											...
											<img
												...
												height='120px'
												className={c.user__photos_item}
												src={myUser.photos[2].src} alt="usersPhoto" />
										</div> : '' : ''}
									</div>
								</div>
								{myUserStatus ?
