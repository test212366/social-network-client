...
const Chat = ({ sendMessage, socket }) => {
	...
	useEffect(() => {
		const api = async () => {
			const responce = await fetch('http://localhost:5000/api/user/getChat', {
				headers: {
				...
				<header className={c.header}>
					...
					<NavLink className={c.noneLink} to={`/profile:${chatUser ? chatUser.userId : ''}`} >
						<div className="user__img">
							<img src={chatUser ? `${chatUser.user.userPhoto != 'false' ? `${chatUser.user.userPhoto}` : "https://okeygeek.ru/wp-content/uploads/2020/03/no_avatar.png"}` : ''}
								alt="user"
								width='34px'
								height='34px'
