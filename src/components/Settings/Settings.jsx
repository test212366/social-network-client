	...
	return (
		<>
			...
			<div className={!close ? ` ${c.contentWrapC} ${c.contentWrap}` : c.contentWrapC}>
				{allPosts ? allPosts.posts.length === 0 ? <p className={c.noneFriends}><img src={info} alt='info' className={c.infoItem} height='20px' width='20px' />Нет записей</p> : '' : ''}
				...
				{userMess ? userMess.length === 0 ? <p className={c.noneFriends}><img src={info} alt='info' className={c.infoItem} height='20px' width='20px' />Нет сообщений</p> : '' : ''}
				{userMess ? userMess.map(message => <Chat props={{ name: message.name, online: message.online, lastMessage: message.lastMessage, userPhoto: message.userPhoto }} />) : ''}
				...
				<div className={c.content}>
					<NavLink to={`/profile:${userMy ? userMy._id : ''}`} className={c.link}>
						<div className={c.profile}>
							{userMy ? <img src={`${userMy.userPhoto != 'false' ? `${userMy.userPhoto}` : "https://okeygeek.ru/wp-content/uploads/2020/03/no_avatar.png"}`} alt="userImg" width='60px' height='60px' className={c.userImg} /> : ''}
